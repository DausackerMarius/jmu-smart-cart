...
=========================================================================================
Dieses Skript liefert den ultimativen betriebswirtschaftlichen Beweis.
Wir simulieren Hunderte von Einkäufen unter realen (von der KI vorhergesagten) 
Verkehrsbedingungen. Wir vergleichen das "Naive Routing" (nur Distanz) mit dem 
"Smart Routing" (Distanz + KI-Stau-Penalties). 

WISSENSCHAFTLICHER ANSATZ (Option A):
Wir nutzen KEIN Fallback-Sicherheitsnetz. Wenn die KI-Heuristik den Kunden in 
seltenen Fällen auf einen längeren Weg schickt, wird dieser Zeitverlust (negative 
Ersparnis) ehrlich erfasst. Das beweist wissenschaftliche Integrität und zeigt das 
reale Risiko/Rendite-Profil des Systems.
=========================================================================================
"""

import matplotlib
matplotlib.use('Agg') # WICHTIG: Verhindert Abstürze ohne Monitor (Headless Mode)
...
# Wir importieren NUR den statischen Graphen aus model.py, um Server-Crashes zu vermeiden!
try:
    import model
    G_base = model.G_base
    nodes_config = model.nodes_config
except ImportError:
    raise ImportError("❌ model.py konnte nicht geladen werden. Bitte sicherstellen, dass sie im gleichen Ordner liegt.")
...
# Business-Metriken (Umrechnung von Personen in verlorene Sekunden)
WALKING_SPEED = 1.2         # Meter pro Sekunde (Normales Gehen)
AISLE_DELAY_SEC = 5.0       # Jede Person im Gang kostet uns 5 Sekunden Zeitverlust (Ausweichen etc.)
QUEUE_DELAY_SEC = 45.0      # Jede Person an der Kasse kostet uns 45 Sekunden Wartezeit
...
def extract_traffic_snapshots():
    """
    Sucht im ungesehenen Testset nach zwei extremen Momenten:
    1. Ruhiger Morgen (Minimaler Traffic)
    2. Extreme Rush-Hour (Maximaler Traffic)
    """
    print("1. Lade Testdaten und suche nach Extrem-Szenarien...")
    ...
    print(f" -> Ruhiges Szenario gefunden: {quiet_ts}")
    print(f" -> Rush-Hour Szenario gefunden: {rush_ts}")
    
    return df, quiet_ts, rush_ts

def predict_snapshot_traffic(df, timestamp, ml_model, features):
    """Nutzt die KI, um das Verkehrsbild für einen exakten Zeitpunkt vorherzusagen."""
...
def build_smart_graph(G, traffic_dict):
    """
    Wandelt den Graphen in einen 'Smart Graph' um.
    Gewicht = Physische Distanz + BPR Penalty (Zeitverlust durch Stau).
    """
    G_smart = G.copy()
...
def quick_tsp(G, start, targets):
    """Autarke Nearest-Neighbor Heuristik für die Wegpunkt-Planung."""
    curr = start
...
def calculate_real_travel_time(route_nodes, G_routing, G_base, traffic_dict):
    """
    Misst die Zeit, die der Agent in der echten Welt (G_base) 
    bei aktuellem Traffic gedauert hätte.
    """
    total_seconds = 0.0
...
def run_monte_carlo():
    ...
    print("\n2. Initialisiere KI-Modell und Graphen...")
    ...
    results = []
    num_simulations = 200 # 200 Einkäufe pro Szenario
    
    print("\n3. Starte Monte-Carlo-Simulation (Agenten-Wettrennen)...")
    for scenario_name, traffic_dict, graph_naive, graph_smart in scenarios:
        ...
        for _ in tqdm(range(num_simulations), leave=False):
            ...
            # WISSENSCHAFTLICHE EHRKEIT: Kein Fallback-Reset! 
            # Wenn time_smart > time_naive, wird die Zeitersparnis ehrlich negativ.
    ...
    # NEU: Gestrichelte Null-Linie als visuelle Basis für Gewinne/Verluste
    ...
    plt.xlabel('Größe des Einkaufs', fontsize=12)
    plt.ylabel('Zeitersparnis in Sekunden (Werte < 0 bedeuten KI-Umweg)', fontsize=12)
    ...
    print(f" -> Durchschnittliche Zeitersparnis (Gesamt):    {df_res['Zeitersparnis_Sekunden'].mean():.1f} Sekunden")
    print(f" -> Durchschnittliche Zeitersparnis (Rush-Hour): {rush_res['Zeitersparnis_Sekunden'].mean():.1f} Sekunden")
    print(f" -> Maximal gemessene Zeitersparnis (Spitze):    {df_res['Zeitersparnis_Sekunden'].max():.1f} Sekunden")
    print(f" -> KI-Erfolgsquote (Rush-Hour):                 {win_rate:.1f}% der Fälle schneller")
    if avg_loss < 0:
        print(f" -> Risiko (Durchschnittlicher Verlust bei Fail): {abs(avg_loss):.1f} Sekunden")
    print("-" * 60)
    print(f"✅ Skript beendet. Der entscheidende Plot liegt in '{OUTPUT_DIR}/business_value_time_saved.png'.")
