...
=========================================================================================
Zweck: Extraktion, NLP-Bereinigung und Transformation des offiziellen 
Bundeslebensmittelschlüssels (BLS) in einen Machine-Learning-bereiten Datensatz.

METHODIK (Data Ingestion & Sanitization):
Diese Pipeline bildet den Ground Truth (Y-Target) für die Logistische Regression (TF-IDF).
Rohdaten aus B2B-Katalogen enthalten massiv Labor-Jargon und Metadaten, die 
Kunden im Supermarkt niemals in ein Tablet eintippen würden. Würden wir das ML-Modell 
auf diesen Rohdaten trainieren, entstünde ein massives Data Leakage (das Modell lernt 
Features, die in der Realität der Inferenz-Phase gar nicht existieren).
Die Pipeline reinigt die Strings, löst Semantik auf und erzeugt saubere (X, Y) Paare.
=========================================================================================
"""
...
# Garantiert, dass die Pipeline auch in schlanken Docker-Containern (CI/CD) nicht abbricht.
try:
...
# =============================================================================
# 1. KONFIGURATION & LEXIKA (Data Governance)
# =============================================================================
...
class Config:
    ...
    # NLP-Filterlisten (Dimensionsreduktion): 
    # B2B-Datenkataloge sind oft bilingual. Ein englisches Wort (z.B. "beef") im String
    # würde die TF-IDF Matrix unnötig aufblähen (Curse of Dimensionality), da unsere
    # Ziel-Domäne (der deutsche Supermarkt) zu 100% deutschsprachig ist.
    EN_KILLER_WORDS = {
    ...
    # B2B_NOISE_WORDS eliminiert toxisches Labor-Rauschen.
    # Ein String wie "Schweineschnitzel, intermuskulär, rohmasse" wird so zu "Schweineschnitzel".
    B2B_NOISE_WORDS = {
...
def setup_logger() -> logging.Logger:
    """
    Erzeugt einen standardisierten StreamHandler für MLOps. 
    Wichtig für sauberes Parsing in Cloud-Logs (z.B. AWS CloudWatch oder ELK-Stack).
    """
...
class BLSDownloader:
    ...
    def download(self) -> bool:
        """
        Lädt die externe B2B-Excel-Datei netzwerksicher herunter.
        Nutzt asynchrones Chunking, um RAM-Overflows bei sehr großen Dateien zu verhindern.
        """
        ...
        try:
            # Ein harter Timeout von 60s verhindert Thread-Deadlocks bei Server-Ausfällen
...
class TextSanitizer:
    ...
    def clean(name: str) -> Optional[str]:
        """
        Das kybernetische Filter-Herz der Pipeline.
        Transformiert Labor-Bezeichnungen in B2C-konsumentenfreundliche Suchbegriffe.
        Verhindert toxisches Data Leakage vor dem ML-Training.
        """
        if pd.isna(name): return None
        name = str(name).strip()
        name_lower = name.lower()
        
        # 1. B2B Filter (Verhindert Overfitting auf Labor-Vokabular)
        if any(noise in name_lower for noise in Config.B2B_NOISE_WORDS): return None
            
        # 2. Englisch Filter (Reduziert Sparse-Matrix Dimensionalität im TF-IDF Modell)
        for en_word in Config.EN_KILLER_WORDS:
            if re.search(r'\b' + en_word + r'\b', name_lower): return None
                
        # 3. Klammer-Annihilation VOR dem Split: Entfernt Meta-Details wie "(gewürzt)"
        name = re.sub(r'\s*\(.*?\)', '', name)
        name = re.sub(r'\s*\[.*?\]', '', name)
            
        # 4. Komma-Cutoff & Signalwort-Cutoff: Extrahiert die semantische Essenz
        name = name.split(',')[0]
        ...
        # 5. Tokenizer-Fix: Ersetzt Slashes durch Leerzeichen. 
        # Verhindert, dass der char_wb Tokenizer harte Wortgrenzen falsch berechnet.
        name = name.replace('/', ' ')
        
        # 6. Zombie-Wörter & Adjektive löschen (Entfernt irrelevante Feature-Weights)
        words_to_strip = [
        ...
        # 7. Isolierte Zahlen löschen (Zahlen erzeugen extrem dünnbesetzte Sparse-Matrizen)
        name = re.sub(r'\b\d+(,\d+)?\b', '', name)
        
        # 8. RETAIL-FILTER: Maximal 3 Wörter erlaubt.
        # Ein "Wort" mit 8 Wörtern ist eine B2B-Beschreibung, kein Produkt, das ein 
        # Kunde sucht. Dieser Filter verhindert eine toxische Class Imbalance.
...
class SupermarketMapper:
    ...
    def generate_label(bls_code: str, original_name: str) -> Optional[str]:
        ...
        Dient als sauberes Y-Target (Ground Truth) für die Logistische Regression.
        """
        bls_code = str(bls_code).strip().upper()
        name_lower = str(original_name).lower()
        
        # Entfernt Supplements, Nahrungsergänzungsmittel und nicht-Lebensmittel
        if bls_code.startswith(('X', 'Y', 'W9', 'U9', 'V9')): return None
            
        # 1. HARTE KEYWORD HEURISTIK (Überschreibt BLS-Codes bei Semantischer Eindeutigkeit)
        ...
        # 2. FALLBACK AUF OFFIZIELLE B2B-NOTATION DES BUNDESLEBENSMITTELSCHLÜSSELS
...
class PipelineOrchestrator:
    ...
    def _locate_target_columns(self) -> Tuple[str, str]:
        """
        Auto-Discovery Heuristik: 
        Verhindert Pipeline-Brüche, falls sich das Spalten-Layout im Excel der Behörde ändert.
        Sucht probabilistisch nach der Spalte mit den meisten gültigen BLS-Regex-Mustern
        und der Spalte mit den semantisch aussagekräftigsten Produktbezeichnungen.
        """
        code_col, name_col = None, None
        
        # Suche nach dem Identifikator (Code)
        ...
        max_score = -1
        # Scoring-System zur Identifikation der Haupt-Bezeichnungsspalte
        for col in self.df_raw.columns:
            ...
            # Belohnt deutsche Sprache und essenzielle Lebensmittel-Keywords
            ...
            # Bestraft harte B2B- oder englische Keywords massiv
    ...
    def run(self) -> bool:
        """
        Führt den Transform-Schritt (T) der ETL-Pipeline durch.
        Wendet die NLP-Heuristiken an und bereitet die CSV für das ML-Training vor.
        """
        try:
            ...
            # Vektorisierte Ausführung mittels pandas apply für performante C-Level Execution
            ...
            # Absolute Notwendigkeit (Deduplikation):
            # Zwei absolut identisch benannte Produkte mit demselben Target würden
            # die TF-IDF Gewichtung verfälschen und die Log-Loss-Berechnung verzerren.
    ...
    def export(self) -> None:
        """Speichert den bereinigten Ground-Truth Datensatz als CSV für das ML-Modell."""
        if self.df_clean is not None:
            ...
            print("="*55)
            # Gibt die Klassenverteilung aus (Wichtig zur Validierung des class_weight='balanced')
