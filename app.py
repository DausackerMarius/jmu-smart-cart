"""
=========================================================================================
PROJEKT: JMU SMART SUPERMARKET - DASHBOARD FRONTEND (ENTERPRISE UI)
=========================================================================================
Dieses Skript bildet das User Interface (UI) der Anwendung, basierend auf Plotly Dash.
Es orchestriert die Interaktion zwischen dem menschlichen Nutzer und den 
Backend-Algorithmen (Graphentheorie, TSP-Solver, XGBoost Traffic Prediction).

ARCHITEKTUR-HINWEIS (Für die Verteidigung / Kolloquium):
Die Applikation implementiert das strenge MVC-Paradigma (Model-View-Controller):
- Model: Ausgelagert in die 'model.py' (Datenhaltung, Graphen, ML-Pipeline).
- View: Das deklarative HTML/Bootstrap-Layout in der Funktion 'serve_layout()'.
- Controller: Die reaktiven '@app.callback' Funktionen, die zustandslos agieren.
=========================================================================================
"""
...
# =============================================================================
# Modulare Architektur: Wir importieren ausschließlich hochgradig gekapselte Objekte 
# aus dem Backend. Dies verhindert zirkuläre Abhängigkeiten und Spaghetti-Code.
from model import (
    nodes_config, 
    edges_config, 
    shelves, 
...
# Kybernetische Schwellenwerte für die M/M/1/K Warteschlangen-Theorie:
# Definiert, ab welchem Systemdruck (Queue Pressure) weitere Ressourcen allokiert werden.
...
# Statischer Start- und Zielknoten (Deterministische Graphen-Grenzen)
G = G_base
s_node = 'vEingang'
e_node = 'vAusgang'

# Initialisierung der UI-Dropdowns für das Admin-Panel.
# Versteckte topologische Hilfsknoten (wie 'vInCorner') werden hier gefiltert, 
# um Dateninkonsistenzen durch Fehlbedienung des Admins zu vermeiden.
GLOBAL_NODE_OPTIONS = [
...
# =============================================================================
# 2. BUSINESS LOGIC LAYER (Traffic Simulation via XGBoost)
# =============================================================================

class TrafficSimulationEngine:
    """
    Diese Klasse kapselt unser trainiertes Machine Learning Modell (XGBoost) für 
    die Stau-Vorhersage. Sie verknüpft Frontend-Zeitangaben mit der Tensor-Prädiktion.
    """
    _model_cache = None
    ...
    def initialize(cls):
        """
        Startup-Routine: Sucht nach dem neuesten kompilierten XGBoost-Artefakt (.pkl)
        und lädt es in den RAM. Dies garantiert Inferenzzeiten von wenigen Millisekunden.
        """
        pkl_files = glob.glob("*.pkl")
        traffic_models = [f for f in pkl_files if "traffic" in f]

        if traffic_models:
            # Automatisches Laden des aktuellsten Modells (Continuous Deployment Support)
    ...
    def get_traffic_state(cls, sim_dt: datetime.datetime) -> tuple[dict, int]:
        """
        Feature Engineering & Inferenz: Füttert das XGBoost-Modell mit 
        temporalen Daten und berechnet die Raum-Zeit-Matrix des Staus.
        """
        ...
        # 1. Temporales Feature Engineering
        month = sim_dt.month
        day = sim_dt.day
        hour = sim_dt.hour
        ...
        # 2. Makro-Simulation des Gesamtmarktes (Sim2Real Approximation)
        if is_christmas and 9 <= hour <= 20:
            total_agents, total_queue, open_registers = 200, 180, 3
        elif is_rush_hour:
            total_agents, total_queue, open_registers = 120, 50, 2
        else:
            total_agents, total_queue, open_registers = 30, 3, 1

        queue_pressure = total_queue / open_registers
        
        # 3. Topologische Graphen-Features extrahieren
        ...
        # 4. Zirkadiane Transformation: 
        # Wir kodieren die Zeit zyklisch über Sinus/Cosinus. Das verhindert die 
        # mathematische Singularität um Mitternacht (wenn 23 Uhr auf 0 Uhr springt) 
        # und erlaubt dem Baum-Algorithmus, kontinuierliche zeitliche Nähe zu lernen.
        feature_pool = {
            'edge_id_enc': df_pred['edge_id_enc'],
            'month': month,  # BUG-FIX 2: Verhindert das Feature-Mismatch-Loch
        ...
        # 5. Inferenz & Re-Transformation
        X_pred = pd.DataFrame({f: feature_pool.get(f, 0) for f in features_list})
        
        # Das Modell sagt logarithmierte (log1p) Werte voraus. Wir wandeln diese 
        # mittels expm1 wieder in greifbare, physikalische Personenanzahlen um.
...
# Initialisiert den XGBoost-Graphen vor dem ersten Web-Request (Kaltstart-Prävention)
TrafficSimulationEngine.initialize()

# =============================================================================
# 3. APP LAYOUT (Frontend-Struktur)
# =============================================================================
# Wir nutzen Dash Bootstrap Components (DBC) für ein responsives CSS-Grid-System.
...
def serve_layout():
    """
    Generiert das HTML-Layout dynamisch. 
    Da das Layout hier eine Funktion ist, wird die Zeit (datetime.now) bei JEDEM 
    Seiten-Reload (F5) frisch gezogen und friert nicht auf der Startzeit des Servers ein.
    """
    ...
    return html.Div([
        ...
        # ---------------------------------------------------------
        # Der Warenkorb (cart-store) wird im Browser des Nutzers (Local Storage) gespeichert.
        # Würden wir Python-Server-Variablen nutzen, würden sich alle Webseiten-Besucher 
        # denselben Korb teilen. Diese Architektur ist O(1) skalierbar.
                    ...
                    dbc.Card([
                        dbc.CardBody([
                            ...
                            html.Div(
                                ...
                                style={'maxHeight': '260px', 'overflowY': 'auto'} # Scroll-Protection bei riesigen Warenkörben
                            ),
...
def handle_routing(pathname):
    ...
    indem Menüs ein- oder ausgeblendet werden, OHNE die Seite serverseitig neu zu laden.
    """
    if pathname == '/ai': 
...
def manage_cart(n_add, n_clear, n_undo, n_submit, text_input, current_cart):
    """
    Verwaltet den Warenkorb-Status (CRUD-Operationen für das Client-Side Dictionary).
    Greift auf die model.py zu, um Produkte mit Fehlertoleranz (Damerau-Levenshtein) zu suchen.
    Integiriert das Auslesen des 'is_frozen'-Flags für das spätere Cold-Chain-Routing.
    """
    ...
    if (trigger_id == 'btn-add-item' or trigger_id == 'input-prod-name') and text_input:
        ...
        if found:
            ...
            # Duplikat-Schutz (Verhindert, dass man 3x zum selben Regal geroutet wird)
            ...
            current_cart.append({
                'node': node_id, 'name': real_name, 'brand': brand, 
                'slot': node_id, 'is_frozen': is_frozen
            })
...
def calculate_turn_direction(p1, p2, p3):
    ...
    Das ist mathematisch robuster und performanter als reine Winkelberechnungen via Arcustangens.
    """
...
def build_true_plot_path(sequence, graph):
    """
    Die TSP-Algorithmen (wie Held-Karp oder Simulated Annealing) geben uns 
    ausschließlich die Reihenfolge der anzusteuernden Regale.
    Diese Funktion füllt die Strecken ZWISCHEN diesen Regalen mit echten Wegpunkten 
    (via Dijkstra) auf, damit die Plotly-Linie im Frontend sauber durch die Gänge 
    navigiert und nicht quer durch die Regale/Wände schneidet.
    """
    ...
    for i in range(len(sequence)-1):
        ...
        try:
            ...
            for j, node in enumerate(sub_path):
                # Verhindert, dass Verbindungsknoten doppelt in die UI-Liste rutschen
    ...
    # CRITICAL BUG FIX (Senior Dev Report #1):
    # Tuple Unpacking repariert
    return px, py
...
def update_visuals(cart_data, n_clicks, view_mode, heat_thresh, current_mode, date_value):
    """
    Der Main-Render-Loop des Dashboards. Wird getriggert, wenn der Nutzer mit 
    dem UI interagiert. Evaluiert die Routen, vergleicht den Sim2Real-Gap 
    (Baseline vs. MLOps) und berechnet den Plotly-Graphen.
    """
    
    cart_data = cart_data or []
    
    # Extraktion der Ziel-Regale. dict.fromkeys() entfernt Duplikate (O(N)), 
    ...
    def format_path_steps(seq, cart, is_ai=False):
        """Übersetzt abstrakte Knoten-IDs in Turn-by-Turn Navigation für das Frontend."""
        steps = []
        for i in range(len(seq)):
            ...
            # Vektor-Kreuzprodukt für die Richtungsanweisung abrufen
            ...
            # UX-Optimierung: Anweisungen basierend auf dem Knoten-Typ
    ...
    # ---------------------------------------------------------
    # UI-GENERIERUNG DES WARENKORBS (inkl. Cold-Chain Badge)
    # ---------------------------------------------------------
    list_items = []
    
    for i in cart_data:
        # Erstellt dynamisch einen blauen Badge, falls das Produkt Tiefkühlware ist.
        # Dies demonstriert dem Nutzer die erfolgreiche ML-Klassifikation und 
        # bereitet das System auf das 'Cold Chain Routing' vor.
        frozen_badge = html.Span([html.I(className="fa-solid fa-snowflake me-1"), "TK-Ware"], className="badge bg-info float-end") if i.get('is_frozen') else ""
        
        list_items.append(html.Div([
            html.Div([
                html.B(i['name']), 
                dbc.Badge(i['slot'], color="primary", className="ms-2"),
                frozen_badge
            ]),
    ...
    for u, v, w in edges_config:
        if u in nodes_config and v in nodes_config:
            ...
            # Der 'None' Wert ist ein topologischer Trick für Plotly, um die Linie 
            # abzubrechen. Ansonsten würde Plotly alle Gänge ununterbrochen verbinden.
    ...
    # WENN der Button geklickt wurde und Produkte im Korb liegen:
    if target_nodes and n_clicks:
        try:
            if current_mode == 'static':
                ...
                # =========================================================
                # Berechnet das klassische Traveling Salesperson Problem rein 
                # nach der euklidischen/manhattan Distanz (Stau-Blindheit).
                ...
                # Extrahieren der Kassenempfehlung (vorletzter Knoten in der Route)
                ...
                # Vorhersage vom ML-Modell abfragen (Raum-Zeit-Tensor)
                loads_dict, total_agents = TrafficSimulationEngine.get_traffic_state(sim_dt)
                
                # Wir erzeugen zwei separate Instanzen des Graphen, um das A/B-Testing 
                # (Sim2Real Gap Evaluierung) durchzuführen.
                G_legal = G_base.copy() # Baseline-Graph (Dummer Agent)
                G_smart = G_base.copy() # BPR-Gepenalisierter Graph (Smater Agent)
                ...
                for u, v, d in G_base.edges(data=True):
                    ...
                    if orig_w < 999999:
                        # Bureau of Public Roads (BPR) Penalty-Funktion:
                        # Verkehrsstau ist nicht linear. Ab einer kritischen Masse 
                        # an Personen explodiert die Verzögerung. Wir nutzen eine 
                        # exponentielle Bestrafung, um das dem Dijkstra-Algorithmus mitzuteilen.
                ...
                # Dynamisches Kapazitäts-Management (M/M/1/K Queue Loss)
                # Ist die Hauptkasse zu voll, öffnet das System virtuelle neue Kassen.
                ...
                # Geschlossene Kassen werden durch Gewicht=Unendlich vom Graph isoliert
                ...
                # A. Dumme Route berechnen (Baseline, läuft blind in Staus)
                _, _, _, seq_b, _ = calculate_hybrid_route(G_legal, s_node, target_nodes, e_node)
                px_b, py_b = build_true_plot_path(seq_b, G_legal)

                # B. Smarte Route berechnen (KI-Agent weicht verstopften Kanten via BPR aus)
                ...
                # Hilfsfunktion zur Fairen Sim2Real Zeitberechnung.
                # Beide Agenten (Dumm vs. Smart) müssen ihre Route im echten 
                # unbarmherzigen Graphen ablaufen und die Stau-Sekunden "bezahlen".
                    ...
                    for i in range(len(seq)-1):
                        ...
                        try:
                            ...
                            for k in range(len(path)-1):
                                ...
                                # Kassenstau kostet massiv Wartezeit
                                if n2 in ['vK1', 'vK2', 'vK3'] and str(n1).startswith('vW'):
                                    q_sec += (r_load * 45.0) 
                                else:
                                    # Normaler Gang-Stau kostet nur Ausweich-Dauer
                ...
                # ---------------------------------------------------------
                # Was passiert, wenn die KI halluziniert und einen 50m Umweg vorschlägt, 
                # um 1 Kunden auszuweichen? Ist die KI in der Simulation langsamer 
                # als die Baseline, zwingt das System den Agenten auf die Baseline zurück.
                # Das garantiert 100%ige Reliability in der Produktion.
                ...
                # Gesamt-Zeitersparnis (Der Return on Investment der MLOps-Architektur)
                ts = tb - tm

                # Heatmap-Rendering: Färbt stark belastete Gänge rot ein (Visual Analytics)
                if view_mode in ['all', 'heat']:
                    for u, v in G_base.edges():
                        real_load = loads_dict.get(f"{u}-{v}", loads_dict.get(f"{v}-{u}", 0.0))
                        if real_load >= heat_thresh:
                            # Alpha-Blending: Skaliert weich von Gelb zu intensivem Rot
                ...
                # Aufbau des Statistik-Panels für das A/B-Testing im Dashboard
                info_box = html.Div([
                    dbc.Card([
                        dbc.CardHeader([
            ...
            # Fehler robust abfangen (Graceful Degradation), damit das Dashboard nie abstürzt
    ...
    for nid, d in nodes_config.items():
        ...
        if nid in target_nodes or nid in [s_node, e_node]:
            ...
            # Numerierung der Regale (1, 2, 3...) im Graphen basierend auf der berechneten TSP-Route
        ...
        # Normale, nicht besuchte Knotenpunkte (werden dezent im Hintergrund als Semi-Transparente Marker gezeichnet)
        elif nid != 'vInCorner':
    ...
    # Optisches Feintuning des Plotly-Graphen (Rahmen, Grid und Achsen vollständig ausblenden)
...
def ai_suggest_slot(n_clicks, product_name):
    ...
    die Entscheidung zwingend manuell verifizieren (Zero-Trust Prinzip).
    """
    if not product_name: 
    ...
    try:
        ...
        # Review-Flag ist True, wenn KI-Sicherheit unter dem 80% (0.8) Threshold liegt.
...
def toggle_admin(n_open, n_login, is_open, pw):
    ...
    Hinweis für die Ausarbeitung: MD5 ist kryptografisch stark veraltet und dient 
    hier lediglich der simplen Demonstration. In einem produktiven System müsste 
    zwingend SHA-256 mit einem Salt verwendet werden, um Rainbow-Table-Angriffe auszuschließen.
    """
    ctx = dash.callback_context
    ...
    if tid == "btn-admin-login":
        ...
        # NARRENSICHERER FIX: Wir berechnen den Hash für 'smartcart2025' dynamisch.
        expected_hash = hashlib.md5("smartcart2025".encode('utf-8')).hexdigest()
        if hashlib.md5(pw.encode('utf-8')).hexdigest() == expected_hash:
            return False, True, "" 
        else:
...
def admin_manage(n_save, n_del, a_node, a_name, a_brand, a_cat, n_rev, del_val):
    ...
    (Server-Abstürze) zu vermeiden, falls ein normaler Nutzer zeitgleich im 
    Dashboard nach einem Produkt sucht, während der Admin speichert.
    """
    ctx = dash.callback_context
    tid = ctx.triggered[0]['prop_id'].split('.')[0] if ctx.triggered else None
    m_add, m_del = "" , ""
        ...
        if a_node and a_name and a_brand:
            try:
                with inv_manager._lock:
                    ...
                    # Generiert das Cold-Chain Flag automatisch basierend auf der Kategorie
                    is_frozen = (a_cat == "Tiefkühlware")
                    
                    inv_manager.stock[a_node].append({
                        'name': a_name, 'brand': a_brand, 'category': a_cat or "Sonstiges",
                        'is_frozen': is_frozen
                    })
                    
                    # Speichern und Such-Index synchron aktualisieren
    ...
    if tid == "btn-admin-delete" and del_val:
        try:
            with inv_manager._lock:
                ...
                # O(N) List-Comprehension filtert das zu löschende Produkt sauber heraus
    ...
    # Aktualisiert die Dropdown-Liste für das Löschen-Fenster dynamisch nach jeder Änderung
...
if __name__ == '__main__':
    # Startet den WSGI/Werkzeug-Entwicklungsserver
