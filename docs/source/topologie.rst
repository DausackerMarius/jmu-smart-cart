    ...
    class StoreTopology:
        ...
        def _hydrate_from_etl(self, config_path: str) -> None:
            ...
            for edge in data["navigable_edges"]:
                u, v, dist = edge["u"], edge["distance_m"]
...
5. Edge-Snapping & Orthogonales Map-Matching
--------------------------------------------
Der Smart Cart lokalisiert sich via IoT-Sensorik. Rohdaten unterliegen jedoch physikalischen Ungenauigkeiten. Würde das UI diese 2D-Koordinaten unbereinigt rendern, sähe der Kunde den Wagen durch Regale gleiten. 

Die Architektur führt daher eine mathematische Dimensionsreduktion durch: Die Koordinate im Raum der Ebene wird orthogonal auf einen 1D-Unterraum (das Liniensegment des Ganges) projiziert (Edge-Snapping).
...
-------------------------------------------------------
Der Dijkstra-Algorithmus operiert auf einem eckigen Wegenetz (90-Grad-Abbiegungen). Physische Einkaufswagen bewegen sich jedoch in interpolierten Kurven. Ein vollbeladener Einkaufswagen hat eine hohe physikalische Trägheit – er kann sich nicht wie ein Roboter auf der Stelle um 90 Grad drehen. Um die Route organisch zu rendern, wendet die Engine **Uniform Catmull-Rom Splines** an. 
