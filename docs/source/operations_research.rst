...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Der A*-Algorithmus ist primär für Point-to-Point Navigation (Ein Start, ein Ziel) konzipiert. Um eine dichte :math:`\mathcal{O}(K \times K)` Distanzmatrix für :math:`K` Zielprodukte aufzubauen, müsste der A*-Algorithmus :math:`K \cdot (K-1)` mal initialisiert werden. 
...
Anstatt :math:`K \cdot (K-1)` Anfragen zu feuern, wird der Algorithmus iterativ nur exakt :math:`K`-mal aufgerufen (Repeated Single-Source Dijkstra) und berücksichtigt dabei strikt die dynamischen BPR-Strafen. Die Zeitkomplexität für den Aufbau der Matrix sinkt dadurch drastisch auf :math:`\mathcal{O}(K \cdot (|V| \log |V| + |E|))`. 
...
-----------------------------------------------
Ein häufiger und fataler Modellierungsfehler in der Literatur ist die Klassifizierung von Supermarkt-Routen als symmetrisches Problem (Symmetric TSP). Ein symmetrisches Problem impliziert, dass die Distanz (oder Zeit) von Knoten A nach B identisch mit dem Rückweg von B nach A ist: :math:`d(A,B) = d(B,A)`.

Diese Annahme ist für das JMU Smart Cart System physikalisch und mathematisch falsch:

* **Topologische Einbahnstraßen:** Wie in der Architektur definiert, besitzen Kassenzonen und Eingangsschranken ausschließlich gerichtete Vorwärtskanten. Ein Zurücknavigieren ist topologisch ausgeschlossen (:math:`d(\text{Kasse}, \text{Regal}) = \infty`).
...
-----------------------------------------------------
Für kleine Warenkörbe (:math:`K \le 11`) verlangt das System absolute Exaktheit. Die Klasse ``HeldKarpDPSolver`` reduziert die Brute-Force-Laufzeit :math:`\mathcal{O}(K!)` durch Dynamische Programmierung auf :math:`\mathcal{O}(K^2 \cdot 2^K)`. 

Das harte Limit von :math:`K \le 11` ist primär durch die exponentielle Raumkomplexität von :math:`\mathcal{O}(K \cdot 2^K)` motiviert. Über diesen Schwellenwert hinaus würde der Memory-Footprint der zu speichernden DP-Tabelle (insbesondere bei 64-Bit Floats) die zustandslosen Backend-Worker unter paralleler Last instabil machen.
...
Dies erlaubt es dem Algorithmus, die bereits besuchten Knoten (``unvisited: frozenset``) direkt als Schlüssel in einem Dictionary (der Memoization-Tabelle) abzulegen. Der Python-Interpreter löst dies intern über hochoptimierte C-Hashmaps in :math:`\mathcal{O}(1)`, was den Server-RAM massiv entlastet.
...
Virtuelle Ameisen schwärmen parallel über den ATSP-Graphen aus. Die Entscheidung einer Ameise :math:`k`, vom aktuellen Produkt :math:`i` zum nächsten Produkt :math:`j` zu navigieren, basiert nicht auf reinem Zufall, sondern auf der hochkomplexen stochastischen Übergangswahrscheinlichkeit :math:`p_{ij}^k`:

.. math::

    p_{ij}^k = \frac{[\tau_{ij}]^\alpha [\eta_{ij}]^\beta}{\sum_{l \in N_i^k} [\tau_{il}]^\alpha [\eta_{il}]^\beta}

Wobei :math:`\tau_{ij}` die auf der Kante liegende Pheromonspur (Erfahrungswert früherer Ameisen) und :math:`\eta_{ij}` die heuristische Sichtbarkeit (der Kehrwert der Entfernung, :math:`\frac{1}{d_{ij}}`) darstellt. :math:`N_i^k` ist die Menge der noch unbesuchten Knoten.
